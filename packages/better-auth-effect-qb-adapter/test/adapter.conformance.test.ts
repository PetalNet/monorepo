...
import { caseInsensitiveTestSuite, normalTestSuite, testAdapter } from "@better-auth/test-utils/adapter";
...
describe("effect-qb Postgres adapter conformance", async () => {
	...
	const runMigrations = async (options: BetterAuthOptions) => {
		...
		await runtime.runPromise(Effect.flatMap(PgClient.PgClient, (sql) => Effect.gen(function* () {
