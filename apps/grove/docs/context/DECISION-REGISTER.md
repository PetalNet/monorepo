...
27. The singleton local Home Host can be claimed only by the operator-pinned browser identity configured for that deployment. Its first verified login binds the Person and Host ownership atomically; other logins do not compete for ownership, and later configuration drift never transfers ownership implicitly.
