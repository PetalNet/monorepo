...
Set `grovePortal=$(jq -r '.links[0].url | rtrimstr("/")' .amp/portals/grove.json)` before using
the commands below.

- Inspect the endpoint inventory with `curl --fail-with-body --silent "$grovePortal/__dev" | jq`.
...
- Diagnose a fresh or running orb with
  `curl --fail-with-body --silent "$grovePortal/__dev/preflight" | jq`. A logged-out browser is
  optional health, not a failed environment.
...
```bash
vp node --use-system-ca apps/grove/scripts/enroll-dev-agent.mjs \
