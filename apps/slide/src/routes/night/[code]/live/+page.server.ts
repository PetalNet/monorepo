...
export const load: PageServerLoad = async ({ params, locals, url }) => {
	...
	if (event.currentPresentationId) {
		...
		currentPresentationVotes = votes.filter((vote) => vote.ratings.length === categoryCount).length;
	}
	...
	if (orderedGroups.length > 0 && event.status === "completed") {
		...
		const firstPlace = sortedGroups.filter((g) => g.totalScore === uniqueScores[0]);
		...
		categoryWinners = event.categories.map((category) => {
			...
			const highestScore = Math.max(...groupScores.map((g) => g.categoryScores[category.id] || 0));
