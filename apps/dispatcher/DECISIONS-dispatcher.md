...
| DP4  | Fence = `lease_generation` per the validated queue prototype; claim/renew/complete/deliver are all single guarded UPDATE statements carrying (worker, fence); reap increments the fence                                                                           | exactly-once effect under reap+reclaim; SQLite single-statement atomicity gives CAS for free                                                                                                       |
