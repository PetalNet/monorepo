//! Internal MCP servers, run in-process via the `--mcp-server <name>` flag.
//!
//! Currently only the `time` server exists: a tiny stdio JSON-RPC loop
//! exposing a `get_current_time` tool. Configuring the bot's own binary as
//! an MCP server command gives the AI plugin a clock without any external
//! dependency.
...
/// Run the named internal MCP server on stdin/stdout until EOF.
///
/// # Panics
///
/// Panics if the current time cannot be formatted as RFC 3339 (should never
/// happen in practice).
