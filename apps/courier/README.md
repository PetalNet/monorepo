...
| Mechanism                                                                                                                                               | Where                                                                           |
| ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Deadline + bounded retries on EVERY external call (sends, media download/upload, lookups, alias resolution, AI/MCP calls)                               | `crates/courier-core/src/bound.rs`, used everywhere                             |
| Per-leg delivery health: per-destination success/failure tracking, pre-registered from config, periodic reports, loud `LEG IS DEAD` escalation, `!diag` | `crates/courier-core/src/health.rs`, `crates/courier/src/engine.rs` (reporter)  |
| Fault isolation: every plugin invocation runs in its own task with a hard budget and panic containment                                                  | `crates/courier-core/src/supervise.rs`, `crates/courier/src/dispatch.rs`        |
| Supervised sync loop: transient errors retry forever with capped backoff; only auth death exits (for restart + re-login)                                | `crates/courier/src/engine.rs`                                                  |
| OS-thread watchdog (exit 70 on stalled sync loop) + startup guard — works even when the async runtime is wedged                                         | `crates/courier-core/src/watchdog.rs`                                           |
| Idempotent relay: persisted per-(event, target) delivery ledger + stable transaction ids; restart/retry never loses or duplicates a message             | `crates/courier-relay/src/ledger.rs`, `crates/courier-relay/src/idempotency.rs` |
| Ledger-driven startup backfill replays exactly what's missing                                                                                           | `crates/courier-relay/src/lib.rs`                                               |
| Per-source-room ordering: relay work is serialized per room via fair locks                                                                              | `crates/courier-relay/src/lib.rs`                                               |
