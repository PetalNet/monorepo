// @ts-nocheck — vendored third-party module (lucide icon map), shipped as-is.
/* PetalNet iconography — lucide ONLY, self-hosted (no CDN), per DESIGN.md §9.
 * Each entry is the inner SVG of a lucide icon (lucide v1.x, ISC). Render via
 * icon(name) → an <svg> string with lucide's standard 24/2/round attrs.
 * If a glyph isn't in lucide it must be added to THIS file (an external file),
 * never inlined ad-hoc in markup. There are no emoji anywhere in the system. */
...
/** Render a lucide icon as an inline <svg> string (24-grid, 2px round strokes). */
export function icon(name, attrs = '') {
  const body = LUCIDE[name];
  if (!body) { console.warn('[icons] missing lucide glyph:', name); return ''; }
  return (
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" ' +
    'stroke="currentColor" stroke-width="2" stroke-linecap="round" ' +
    'stroke-linejoin="round" aria-hidden="true" ' + attrs + '>' + body + '</svg>'
  );
}

/** Replace any element carrying [data-icon="name"] with that lucide glyph. */
...
/** A data-URI of a lucide icon stroked in `color` — for CSS backgrounds / <img>. */
export function iconDataUri(name, color) {
  const body = LUCIDE[name];
  if (!body) return '';
  return 'data:image/svg+xml;utf8,' + encodeURIComponent(
    "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' " +
    "stroke='" + color + "' stroke-width='2' stroke-linecap='round' " +
    "stroke-linejoin='round'>" + body.replace(/"/g, "'") + '</svg>'
  );
}
