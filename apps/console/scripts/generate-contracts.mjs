...
const repoDir = path.resolve(consoleDir, "../..");
const contractDir = path.join(repoDir, "apps/console-api/docs/contracts");
...
async function emit(relative, content) {
	...
	if (check) {
		...
		if (existing !== normalized)
			throw new Error(`${path.relative(repoDir, file)} is stale; run contracts:generate`);
	} else {
