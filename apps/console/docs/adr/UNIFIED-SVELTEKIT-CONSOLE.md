...
- zod is gone (validators in 9 domain modules, the API core, and 12 remotes). SER `Query` and `Command` remotes take Effect Schema natively, with no Standard-Schema bridge. `src/lib/contracts/schema-conventions.ts` replicates zod's exact accept/reject semantics (strict/loose structs via `parseOptions` annotations, calendar-aware datetime regexes, defaults via `withDecodingDefault`).
...
- `docs/contracts/schemas/**` (34 files), the contract generator script, and the codegen test are deleted; ops.json is self-contained (its two file $refs inlined) and remains live because both the UI operation registry and server API decode it at startup. Bus client frames validate through `@petalnet/console-bus-rpc`'s `BusClientFrameSchema` — tightened to mirror the deleted bus-frame JSON exactly (reject unknown keys, glob-pattern regex, since ≥ 0). Pipeline contract assertions decode against the SSOT schemas (constraints transcribed, nothing weakened).
...
- knip is green in both modes: deploy scripts + effectdb config are production entries (`!`), tests are explicit development entries, and only type/interface exports used in their declaring file receive the type-only exemption.

### Phase 5 — fontless actually emits Geist

- Fontless uses its default `"font-prefixed-only"` scan against the design system's `--font-sans`, `--font-mono`, and `--font-serif` variables.
...
### Phase 8 — CI green + final verify (2026-07-19)

- Grep-proofs over apps/console, all zero: the legacy server-framework name (docs included), `import postgres`/`postgres.Sql`/`txSql`, `from "zod"` in src+scripts, the contract-generator name, `from "ws"` in src. package.json lists none of the removed deps.
- Gates, run locally exactly as CI does: `vp run check` (recursive typecheck + repo lint + fmt) ✓; `manypkg` ✓; `dedupe --check` ✓; `typesync:check` ✓; `lint:knip` ✓; **`lint:knip:prod --strict` ✓** (the previously red gate — fixed at the real layers: deploy scripts/effectdb config as production entries, effect-db/effect-qb as runtime deps, crossws type-only devDep, committed tsc artifacts removed, in-file-used exports not counted dead); `vp run build` ✓ (with CI's collegemap DATABASE_URL env); typos ✓ (`SER` allowed as the svelte-effect-runtime acronym — pre-existing repo-wide flag); repo-wide lychee ✓ (0 errors); zizmor ✓ (no findings).
- The complete test suite runs under Vitest, including the former standalone Node test files and the real TimescaleDB integration tests.
- End-to-end drive of the built server (`node build/index.js`): /login 200, health/attention/query/mcp/openapi all serve, /op dry-run returns the audited envelope (audit_seq, executor evidence, capabilities), a real op against an unconfigured executor fails honestly with 503 executor semantics, and the SvelteKit-owned bus WebSocket answers an unauthenticated upgrade with the contract `unauthorized` ack. Bus subscribe/event/resync/grant-refence/unsubscribe are covered end-to-end by the typed-client suite (test/domain/bus-rpc.test.ts) and release-acceptance.
