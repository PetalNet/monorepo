...
export function compareCostPairWith(
	...
	return Effect.gen(function* () {
		const usageRequest: Parameters<typeof runStructured>[2] = {
			...
			from: "usage_events",
			group_by: [
				...new Set([input.dimension, "model", "session_id", "cost_status", "cost_source"]),
			],
			select: [
		...
		const [usage, pricing] = yield* Effect.all([run(usageRequest), run(pricingRequest)], {
		...
		const matchRate = (model: string) => {
			...
			const normalizedMatch = priceRows.find(
				({ pattern }) => normalizeModel(pattern) === normalized,
			);
