...
import { runStructured, type QueryResult } from "../query/structured.ts";
...
/** The recoverable failure the dashboard/Library surface models: a caller-fixable request or authz. */
...
function rebindQueryRef(
	app: Sql,
	authorScopes: readonly string[],
	targetScope: string,
	queryRef: string,
): Effect.Effect<QueryResult, DashboardError> {
	return Effect.gen(function* () {
		const record = yield* readQueryRecord(app, authorScopes, queryRef);
		if (!record)
			return yield* Effect.fail(
				new DashboardError("query_not_visible", "one or more panel query refs are not visible"),
			);
		// Re-execution under only the dashboard scope proves that every future holder of this scope can
		// resolve the ref. The saved artifact never inherits the author's broader grant set. A query the
		// dashboard scope cannot execute (QueryError) is not shareable; an infra fault stays a defect.
		return yield* Effect.mapError(
			runStructured(app, [targetScope], record.request),
			() =>
				new DashboardError(
					"query_not_shareable",
...
function rebindDashboardPayload(
	app: Sql,
	authorScopes: readonly string[],
	targetScope: string,
	input: DashboardInput,
): Effect.Effect<DashboardPayload, DashboardError> {
	return Effect.gen(function* () {
		const rebound = new Map<string, QueryResult>();
		const bind = (ref: string): Effect.Effect<QueryResult, DashboardError> =>
			Effect.gen(function* () {
				const known = rebound.get(ref);
				if (known) return known;
				const result = yield* rebindQueryRef(app, authorScopes, targetScope, ref);
				rebound.set(ref, result);
				return result;
			});
		const panels: PanelSpecV2[] = [];
		for (const raw of input.panels as unknown as PanelSpecV2[]) {
			...
			if (raw.query_ref) {
				const result = yield* bind(raw.query_ref);
			...
			if (typeof raw.prose === "string") {
				...
				for (const match of statBindings(raw.prose)) {
					...
					const result = yield* bind(originalRef);
		...
		if (branch?.selected_mark) {
			...
			if (selectedRef) branch.selected_mark.query_ref = (yield* bind(selectedRef)).query_ref;
		}
...
export function saveDashboard(
	...
	principal: Principal,
	input: DashboardInput,
): Effect.Effect<Record<string, unknown>, DashboardError> {
	return Effect.gen(function* () {
		const scope = dashboardTargetScope(principal, input.scope);
		if (!scope || !principal.scopes.includes(scope))
			return yield* Effect.fail(
				new DashboardError("scope_denied", "dashboard scope is not visible to the caller"),
			);
		if (!(yield* Effect.promise(() => canMutateScope(db.writer, principal, scope))))
			return yield* Effect.fail(
				new DashboardError("scope_denied", "editor relation required for the dashboard scope"),
			);
		const hash = requestHash(input);
		const existing = yield* Effect.promise(
			() => db.writer<MutationRow[]>`
		...
		if (existingMutation) {
			if (existingMutation.request_hash !== hash)
				return yield* Effect.fail(
					new DashboardError("id_reused", "mutation id was already used with a different body"),
				);
			const item = yield* Effect.promise(() =>
				dashboardById(db.writer, existingMutation.dashboard_id),
			);
			if (!item) return yield* Effect.die(new Error("dashboard mutation points to a missing item"));
		...
		const payload = yield* rebindDashboardPayload(db.app, principal.scopes, scope, input);
		const id = `dash_${createHash("sha256").update(`${principal.id}\0${input.id}`).digest("hex").slice(0, 24)}`;
		// The idempotent claim + insert is one transactional edge that raises only DashboardError; a
		// dangling mutation pointer (the plain Error below) is an invariant break, so it dies.
		return yield* edge(isDashboardError, () =>
			db.writer.begin(async (tx) => {
				...
				where principal_id = ${principal.id} and request_id = ${input.id}`;
					...
					if (!previous || previous.request_hash !== hash)
						throw new DashboardError(
							"id_reused",
							"mutation id was already used with a different body",
						);
					...
					if (item.scope !== scope || !principal.scopes.includes(item.scope))
						throw new DashboardError(
							"scope_denied",
							"dashboard scope is not visible to the caller",
						);
					return itemEnvelope(item);
				}
...
export function listDashboards(
	...
	// An unparseable/forged cursor is the one caller-fixable failure; the scoped read is an infra edge.
	return edge(isDashboardError, async () => {
...
function readDashboardRow(
	app: Sql,
	scopes: readonly string[],
	id: string,
): Effect.Effect<ItemRow | null> {
...
export function materializeTextPanel(
	app: Sql,
	scopes: readonly string[],
	panel: PanelSpecV2,
): Effect.Effect<MaterializedPanel> {
	return Effect.gen(function* () {
		...
		for (const match of statBindings(panel.prose)) {
			...
			const record = yield* readQueryRecord(app, scopes, queryRef);
			if (!record) {
				bindings.push({ binding, query_ref: null, column, value: null, status: "refused" });
				continue;
			}
			// A query the viewer cannot execute (QueryError) is refused, not fatal; an infra fault stays
			// a defect and propagates untouched (only the QueryError channel is caught to `null`).
			const outcome = yield* runStructured(app, scopes, record.request).pipe(
		...
		const out: MaterializedPanel = { schema_version: 1, panel: rendered, result: null, render };
		return out;
	});
}
...
export function loadDashboard(
	...
	return Effect.gen(function* () {
		const row = yield* readDashboardRow(app, scopes, id);
		if (!row) return null;
		const materialized: MaterializedPanel[] = [];
		for (const panel of row.payload.panels) {
			const queryRef = panel.query_ref;
			if (!queryRef) {
				if (panel.type === "text")
					materialized.push(yield* materializeTextPanel(app, scopes, panel));
				else
					materialized.push(
						panel.type === "refusal" ? nativePanel(panel) : unavailablePanel(panel),
					);
				continue;
			}
			const record = yield* readQueryRecord(app, scopes, queryRef);
			...
			// A panel query the viewer cannot execute (QueryError) degrades to an unavailable panel; an
			// infra fault stays a defect and propagates untouched (only the QueryError channel is caught).
			const outcome = yield* runStructured(app, scopes, record.request).pipe(
				Effect.catch(() => Effect.succeed(null)),
			);
			materialized.push(
				outcome === null ? unavailablePanel(panel) : materializePanel(panel, outcome),
			);
		}
...
export function setHomeDashboard(
	writer: Sql,
	principal: Principal,
	id: string,
): Effect.Effect<Record<string, unknown>, DashboardError> {
	// Owner-only home toggle: not-found and non-owner are the caller-fixable failures; the writes die.
	return edge(isDashboardError, async () => {
...
export function listLibraryItems(
	...
	// An unknown kind or a forged cursor are the caller-fixable failures; the ranked read is infra.
	return edge(isDashboardError, async () => {
...
export function searchLibraryPaletteItems(
	app: Sql,
	scopes: readonly string[],
	query: string,
	limit = 32,
...
export function readLibraryItem(
	app: Sql,
	scopes: readonly string[],
	itemId: string,
): Effect.Effect<Record<string, unknown> | null> {
...
export function readLibraryItemHistory(
	app: Sql,
	scopes: readonly string[],
	itemId: string,
): Effect.Effect<Record<string, unknown>> {
...
export function updateLibraryItemStatus(
	...
	// A disallowed target status is the caller-fixable failure; the register write is one infra edge.
	return edge(isDashboardError, () =>
...
export function listLibraryLinks(
	app: Sql,
	scopes: readonly string[],
	cursorSecret: string,
	itemId?: string,
	...
	// A forged page cursor is the one caller-fixable failure; the scoped link read is an infra edge.
	return edge(isDashboardError, async () => {
		const limit = libraryLimit(opts.limit);
		const fingerprint = createHash("sha256")
...
export function listLibraryHolds(
	app: Sql,
	scopes: readonly string[],
	principalId: string,
	cursorSecret: string,
	...
	// A forged page cursor is the one caller-fixable failure; the scoped holds read is an infra edge.
	return edge(isDashboardError, async () => {
		const limit = libraryLimit(opts.limit);
		const fingerprint = createHash("sha256")
...
export function listLibraryCuration(
	app: Sql,
	scopes: readonly string[],
	cursorSecret: string,
	opts: LibraryPageOptions = {},
): Effect.Effect<Record<string, unknown>, DashboardError> {
	// A forged page cursor is the one caller-fixable failure; the curation read is an infra edge.
	return edge(isDashboardError, async () => {
		const limit = libraryLimit(opts.limit);
...
export function listLibraryCapabilities(
	app: Sql,
	scopes: readonly string[],
	cursorSecret: string,
	opts: LibraryPageOptions = {},
): Effect.Effect<Record<string, unknown>, DashboardError> {
	// A forged page cursor is the one caller-fixable failure; the registry projection read is infra.
	return edge(isDashboardError, async () => {
		const limit = libraryLimit(opts.limit);
