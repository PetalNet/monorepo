...
export const initializeAdminBootstrap = async (authentication: typeof auth) => {
	...
	try {
		await context.adapter.transaction(async (adapter) => {
			const admins = await adapter.count({
				model: "user",
				where: [{ field: "tier", value: "owner" }],
			});
			if (admins > 0) return;
			const pending = await adapter.findOne({
				model: "verification",
...
export const redeemAdminBootstrap = async (
	...
	return context.adapter.transaction(async (adapter) => {
		if (await adapter.count({ model: "user", where: [{ field: "tier", value: "owner" }] }))
			return false;
		const pending = await adapter.findOne<{ id: string; value: string }>({
			model: "verification",
		...
		const consumed = await adapter.update({
			model: "verification",
			where: [
				{ field: "id", value: pending.id },
				{ field: "value", value: pending.value },
			],
			update: { value: randomBytes(24).toString("base64url"), updatedAt: new Date() },
		});
		if (!consumed) return false;
		const user = await adapter.update({
			model: "user",
			where: [{ field: "id", value: userId }],
			update: { tier: "owner", updatedAt: new Date() },
		});
