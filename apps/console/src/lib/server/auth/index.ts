...
const issuer = process.env.OIDC_ISSUER ?? (building ? "https://oidc.invalid" : required("OIDC_ISSUER"));
...
export const auth = betterAuth({
	...
	user: { additionalFields: { tier: { type: "string", required: true, defaultValue: "viewer", input: false } } },
	plugins: [
		genericOAuth({
			config: [
				{
					...
					pkce: true,
					scopes: (process.env.OIDC_SCOPES ?? "openid,profile,email").split(",").map((scope) => scope.trim()).filter(Boolean),
				},
			],
		}),
