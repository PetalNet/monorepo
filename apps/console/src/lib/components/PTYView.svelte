...
<section class="pty" aria-label={`Read-only terminal for ${session.handle ?? session.host}`}>
	<header>
		...
		<span class="owner"
			...
			<b>{session.handle ?? "resident"}</b></span
		>
	...
	<div class="viewport" role="log" aria-live="off" aria-label="Terminal output">
		...
		{:else if state === "connecting"}
			<span class="muted"
				>Connecting to {session.host} · {session.tmux_session}:{session.pane_id}</span
			>
		{:else}
			<div class="failure">
				<span
					>{state === "ended"
						? "Session ended."
						: `${errorCode ?? "pty_unavailable"} · No terminal output is available.`}</span
				>
				{#if state === "error" && onretry}<button type="button" onclick={onretry}>Retry</button
					>{/if}
			</div>
		{/if}
	</div>
