...
<div class="cockpit-loading" role="status" aria-label="Loading cockpit">
	...
	<div class="chip-row" aria-hidden="true">
		{#each Array.from({ length: COCKPIT_SKELETON.chips }, (_, index) => index) as index (index)}<span
				class="chip"
			></span>{/each}
	</div>
		...
		<section class="town" aria-hidden="true">
			...
			{#each Array.from({ length: COCKPIT_SKELETON.attentionRows }, (_, index) => index) as index (index)}
				<div class="attention">
					<span class="dot"></span>
					<div>
						<span class="line primary" style:width={index === 1 ? "62%" : "72%"}></span><span
		...
		<aside class="houses" aria-hidden="true">
			...
			<div class="house-row">
				{#each Array.from({ length: COCKPIT_SKELETON.houseTiles }, (_, index) => index) as index (index)}
					<div class="house">
						<span class="roof"></span><span class="body"><i></i><i></i></span><span class="name"
						></span>
					</div>
				{/each}
			</div>
...
<style>
	...
	.body,
	.name {
		animation: skeleton-shift 1.2s ease-in-out infinite alternate;
	}
