<script lang="ts">
	...
	const actions = $derived<PaletteItem[]>([
		{
			...
			kind: "action",
			label: "Ask Janet",
			description: "Focus the assistant",
			meta: "/",
		},
		{
			id: "action:attention",
			...
			description: "Open Cockpit attention",
			href: "/#attention",
			meta: "G C",
		},
		{
		...
		},
		{
			...
			description: "Open the one store",
	...
	function listKey(event: KeyboardEvent) {
		...
		queueMicrotask(() =>
			document
				.getElementById(`palette-option-${String(selected)}`)
...
>
	<div class="palette-shell">
		...
		>
			...
			{#if loading}
				<div class="loading" role="status" aria-live="polite">
					<span></span><span></span><span></span><em>Searching visible objects</em>
				</div>
			{:else if failed || (!connected && text.trim())}
				<div class="message warn" role="status">
					<Icon name="circle-help" size={16} /><span
						>Object search is unavailable. Surfaces and quick actions still work.</span
					>
				</div>
			{:else if text.trim() && items.length === 0}
				<div class="message" role="status">
					<Icon name="search" size={16} /><span
						>No visible matches. Try an agent handle, task number, host, or surface.</span
					>
				</div>
			{/if}
		...
		<footer>
			<span><kbd>↑</kbd><kbd>↓</kbd> move</span><span><kbd>↵</kbd> open</span><span
				><kbd>Esc</kbd> close</span
			><span class="scope">Only objects in your scope appear.</span>
		</footer>
	</div>
</ModalSurface>
...
<style>
	...
	.selected .result-icon {
		background: color-mix(in srgb, var(--petal) 14%, var(--s1));
	...
	.results > button.selected > :global(svg) {
		opacity: 1;
		transform: none;
	}
	.loading {
	...
	.loading span {
		...
		border-radius: var(--r-sm);
		background: var(--s2);
		animation: sheen 1.2s ease-in-out infinite alternate;
	}
