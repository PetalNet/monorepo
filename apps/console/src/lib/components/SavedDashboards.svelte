<script lang="ts">
	...
	/**
	 * Saved dashboards beneath the ask box (foundations §5.5): dashboards are Library items (kind:
	 * artifact). Empty copy from the lore pack.
	 */
	interface Props {
	...
	$effect(() => {
		...
		try {
			order = JSON.parse(localStorage.getItem(storageKey) ?? "[]") as string[];
		} catch {
			order = [];
		}
		orderReady = true;
	...
	async function share(id: string) {
		...
		try {
			...
			snackbar.push({
				message: `dashboard.share failed: ${(error as Error).message}`,
				op: "dashboard.share",
				tone: "danger",
			});
		} finally {
	...
	async function load(event: MouseEvent, id: string) {
		...
		try {
			...
			snackbar.push({
				message: `dashboard.load failed: ${message}`,
				op: "dashboard.load",
				tone: "danger",
			});
		} finally {
	...
	{:else}
		<div class="saved-row">
			{#each orderedItems as d, index (d.id)}
				<article
					class="saved-tile"
					class:loading={loading === d.id}
					aria-busy={loading === d.id}
					data-ask="saved dashboard {d.name}"
				>
					<a
						href="/observability?dashboard={d.id}"
						aria-label="Load {d.name}"
						onclick={(event) => void load(event, d.id)}
					>
						<b>{d.name}</b><span>{d.sub}</span><small>{provenance(d)}</small>
					</a>
					<details class="curate">
						<summary aria-label="Curate {d.name}" title="Dashboard actions"
							><Icon name="ellipsis" size={16} /></summary
						>
						<div class="menu">
							<div class="reorder" aria-label="Rearrange dashboard">
								<button
									...
									onclick={() => {
										move(d.id, -1);
									}}><Icon name="arrow-left" size={14} />Earlier</button
								>
								<button
									...
									onclick={() => {
										move(d.id, 1);
									}}>Later<Icon name="arrow-right" size={14} /></button
								>
							</div>
							...
							{#if lanes.includes("viewer")}
								<button
									class="menu-action"
									onclick={() => (sharing = sharing === d.id ? null : d.id)}>Share</button
								>
							{/if}
					...
					{#if sharing === d.id}
						...
						>
							...
							<div>
								<input
									id="share-{d.id}"
									bind:value={shareSubject}
									placeholder="principal or tier:name"
									required
								/><button disabled={shareBusy}>{shareBusy ? "Sharing…" : "Grant view"}</button>
							</div>
							<small>Viewers re-run every panel inside their own scope.</small>
						</form>
					{/if}
					{#if loadError?.id === d.id}<p class="load-error" role="status">
							Could not load. {loadError.message}
						</p>{/if}
				</article>
			{/each}
		</div>
...
<style>
	...
	.saved-tile.loading {
		background: var(--s2);
		animation: tile-loading 900ms ease-in-out infinite alternate;
	}
	.saved-tile > a {
		display: block;
		color: inherit;
	...
	.curate {
		position: absolute;
		inset: var(--s-1) var(--s-1) auto auto;
	}
	.curate summary {
		display: grid;
		place-items: center;
	...
	.curate summary::-webkit-details-marker {
		display: none;
	}
	.curate summary:hover,
	.curate[open] summary {
		background: var(--s3);
		color: var(--text);
	}
	.curate summary:focus-visible,
