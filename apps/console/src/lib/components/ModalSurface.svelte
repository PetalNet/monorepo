...
<dialog
	bind:this={element}
	class="modal-surface {variant} {size}"
...
<style>
	...
	.modal-surface:not([open]) {
		display: none;
	}
	.modal-surface::backdrop {
		background: color-mix(in srgb, var(--text) 24%, transparent);
	}
	.modal-surface :global(.dialog-close) {
		position: absolute;
		right: var(--s-3);
		top: var(--s-3);
	}
	.dialog {
		...
		max-height: calc(100dvh - var(--s-5));
		margin: auto;
		background: var(--s2);
	...
	.palette {
		width: 640px;
		max-width: calc(100% - var(--s-5));
		max-height: min(680px, calc(100dvh - var(--s-5)));
