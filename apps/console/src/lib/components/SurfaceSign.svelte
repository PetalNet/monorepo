<script lang="ts">
	...
	/**
	 * SurfaceSign (foundations §3.7): sign-face title + jade fine line + optional HUD chips, the 40px
	 * signage row. On the cockpit the fine line is the facade honesty hero — the exact pilot string
	 * renders ONLY on positive evidence (§4.6); a P0 fractures it, silence shows "Can't verify".
	 */
	...
	<div class="crack-row" role="status" aria-live="polite">
		<div class="crack-hero">
			...
			{#if leadDef && crackLead}
				<div class="crack-action">
					<OpButton def={leadDef} args={crackLead.args} {lanes} variant="primary" />
				</div>
			{/if}
		</div>
...
<style>
	...
	h1 {
		font: 400 1.25rem/1.15 var(--sign);
