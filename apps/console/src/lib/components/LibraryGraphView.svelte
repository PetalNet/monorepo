<script lang="ts">
	...
	const neighbors = $derived(
		new Set(focusedId ? (links[focusedId] ?? []).map(({ targetId }) => targetId) : []),
	);
	const icons = {
		task: "circle-check",
		project: "folder",
	...
	function walk(event: KeyboardEvent, id: string) {
		const directions: Partial<Record<string, "left" | "right" | "up" | "down">> = {
			ArrowLeft: "left",
			ArrowRight: "right",
			ArrowUp: "up",
			ArrowDown: "down",
	...
	function edgePath(fromId: string, toId: string) {
		...
		if (!from || !to) return "";
		const bend = Math.max(48, (to.x - from.x) * 0.48);
...
<section class="graph-surface" aria-label="Typed-link dependency graph">
	<header>
		<div>
			...
			<p>{items.length} scope-filtered items · arrows walk typed links</p>
		</div>
		<div class="graph-controls" aria-label="Graph zoom controls">
			<button
				...
				aria-label="Zoom out"><Icon name="zoom-out" size={15} /></button
			>
			...
			<button
				...
				aria-label="Zoom in"><Icon name="zoom-in" size={15} /></button
			>
			<button onclick={reset} aria-label="Reset graph position"
				><Icon name="locate-fixed" size={15} /></button
			>
		</div>
	</header>
	{#if degraded}<p class="degraded">
			<Icon name="triangle-alert" size={14} />Link index is stale. Relationships are last-known and
			drawn dashed.
		</p>{/if}
	{#if loading}
		<div class="graph-loading" aria-label="Loading dependency graph">
			{#each Array.from({ length: 7 }, (_, index) => index) as index (index)}<span
					style={`left:${String(8 + (index % 4) * 23)}%;top:${String(16 + Math.floor(index / 4) * 40)}%`}
				></span>{/each}
		</div>
	{:else if items.length === 0}
		<div class="empty">
			<Icon name="git-branch" size={20} /><b>The stacks are open.</b><span
				>Nothing filed in this scope yet.</span
			>
		</div>
	{:else}
		<div class="phone-list">
			{#each items as item (item.id)}<LibraryItemCard {item} compact {onopen} />{/each}
		</div>
		<div
			bind:this={viewport}
			class:dragging
			class="viewport"
			role="group"
			aria-label="Pan and zoom dependency graph"
			onwheel={wheel}
		...
		>
			<div
				class="canvas"
				style={`width:${String(graph.width)}px;height:${String(graph.height)}px;transform:translate(${String(panX)}px,${String(panY)}px) scale(${String(scale)})`}
			>
				<svg width={graph.width} height={graph.height} aria-hidden="true">
					<defs
						><marker
							...
							refX="6"
							refY="3.5"
							orient="auto"><path d="M0,0 L7,3.5 L0,7 Z" /></marker
						></defs
					>
					{#each graph.edges as edge (`${edge.from}-${edge.to}-${edge.rel}`)}
						<g class:active={focusedId === edge.from || focusedId === edge.to}>
							<path
								class:stale={degraded}
								d={edgePath(edge.from, edge.to)}
								marker-end="url(#library-arrow)"
							/>
							<text
								x={(nodeById.get(edge.from)?.x ?? 0) + 104}
								y={((nodeById.get(edge.from)?.y ?? 0) + (nodeById.get(edge.to)?.y ?? 0)) / 2 - 7}
								>{edge.rel}</text
							>
						</g>
					{/each}
				...
				{#each graph.nodes as node (node.id)}
					<button
						...
						class:neighbor={neighbors.has(node.id)}
					...
					>
						<span class="node-icon"><Icon name={icons[node.kind]} size={16} /></span><span
							class="node-title">{node.title}</span
						>
					</button>
				{/each}
			</div>
...
<style>
	...
	.node-icon {
		...
		background: var(--bg);
		color: var(--info-text);
		box-shadow: inset 0 0 0 2px var(--info-dot);
	}
	.node.task .node-icon,
	...
	.node.research .node-icon {
		...
		box-shadow: inset 0 0 0 2px var(--research-text);
	}
	.node.decision .node-icon {
		color: var(--warn-text);
		box-shadow: inset 0 0 0 2px var(--warn-dot);
	}
