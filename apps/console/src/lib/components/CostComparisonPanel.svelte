...
<div class="compare-shell" aria-live="polite">
	...
	{:else if result}
		...
		<details class="receipt">
			...
			<p>
				<b>Cost source · {result.receipt.cost_source}</b><br />Provider-reported cost wins per usage
				row; otherwise the four token kinds are multiplied by the effective model rates.
			</p>
			{#if result.receipt.pricing.models.length}
				<div class="rate-table" role="table" aria-label="Effective model rates">
					<div class="rate-head" role="row">
						<b role="columnheader">Model → matched pattern</b><span role="columnheader"
							>in / out / write / read · USD/M</span
						>
					</div>
					{#each result.receipt.pricing.models as rate, __eachKey8 (__eachKey8)}
						<div role="row">
							<span role="cell"><code>{rate.model} → {rate.matched_pattern}</code></span><span
								role="cell"
								><code
									>{rate.input_per_mtok.toFixed(2)} / {rate.output_per_mtok.toFixed(2)} / {rate.cache_creation_per_mtok.toFixed(
										2,
									)} / {rate.cache_read_per_mtok.toFixed(2)}</code
								></span
							>
						</div>
					{/each}
			...
			<dl>
				...
				<div>
					<dt>Source</dt>
					<dd>{result.receipt.source}</dd>
				</div>
				<div>
					<dt>Ledger query</dt>
					<dd>{result.query_ref}</dd>
				</div>
				<div>
					<dt>Price book</dt>
					<dd>{result.pricing_query_ref}</dd>
				</div>
				<div>
					<dt>Price version</dt>
					<dd>{result.receipt.pricing.table_version}</dd>
				</div>
				<div>
					<dt>Price digest</dt>
					<dd>{result.receipt.pricing.digest}</dd>
				</div>
				<div>
					<dt>Result</dt>
					<dd>
						{result.receipt.row_count} aggregate rows · {result.receipt.session_count} sessions · {result
							.receipt.execution_ms ?? "—"} ms
					</dd>
				</div>
				<div>
					<dt>Counted through</dt>
					<dd>{new Date(result.observed_at).toLocaleString()}</dd>
				</div>
				<div>
					<dt>Query</dt>
					<dd>{result.receipt.query}</dd>
				</div>
			</dl>
			<div class="receipt-actions">
				<button type="button" onclick={copyQuery}
					><Icon name="copy" size={14} />{copied ? "Copied" : "Copy query"}</button
				>
...
<style>
	...
	.metric {
		display: grid;
		grid-template-columns: minmax(104px, 1fr) minmax(88px, 1fr) minmax(88px, 1fr) minmax(
				88px,
				0.8fr
			);
		gap: var(--s-2);
	...
	.rate-table [role="cell"]:last-child {
		text-align: right;
	}
	.rate-head {
		color: var(--text-3);
	...
	@media (max-width: 520px) {
		.pair-head,
		.metric {
			grid-template-columns: minmax(88px, 1fr) 72px 72px;
		}
		.pair-head > *:last-child,
		.metric > span {
			grid-column: 2/-1;
		}
		.metric > span {
