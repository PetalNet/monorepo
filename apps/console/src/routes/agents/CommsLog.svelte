<script lang="ts">
	...
	function selectType(value: TypeFilter): void {
		type = value;
	}

	function kind(item: CommsEvent): Exclude<TypeFilter, "all"> {
		return item.method === "comms.card"
			? "task-card"
			: item.method === "comms.rpc"
				? "rpc"
				: "mail";
	}
...
<section class="comms-log" aria-labelledby="correspondence-title">
	<header>
		...
		<span class="truth" class:error={failed} role="status" aria-live="polite">
			{failed
				...
				: loading
					? "Reading the archive"
					: `${String(items.length)} ${items.length === 1 ? "letter" : "letters"}`}
		</span>
	</header>
	...
	<div class="table-wrap" aria-busy={loading}>
				...
				{#if loading}
					{#each Array.from({ length: 6 }, (_, index) => index) as index (index)}
						<tr class="skeleton-row" aria-hidden="true"
							><td><span></span></td><td><span></span></td><td><span></span></td><td
								><span></span></td
							><td><span></span></td></tr
						>
					{/each}
				{:else if taskInvalid}
					<tr
						><td colspan="5" class="state"
							><Icon name="circle-alert" size={15} />Task ID must be a positive integer.</td
						></tr
					>
				{:else if failed}
					<tr
						><td colspan="5" class="state"
							><Icon name="circle-help" size={15} />Query failed. Nothing rendered, nothing
							pretended. <button type="button" onclick={() => void read()}>Try again</button></td
						></tr
					>
				{:else if items.length === 0}
					<tr
						><td colspan="5" class="state"
							><Icon name="mailbox" size={15} />No letters in this window.</td
						></tr
					>
				{:else}
					{#each items as item (item.id)}
						<tr>
							...
							<td data-label="Route"
								><div class="route">
									<AgentPresence handle={item.sender} label="from" /><span aria-hidden="true"
										>→</span
									><AgentPresence handle={item.recipient} label="to" />
								</div></td
							>
							<td data-label="Type"><span class="type-chip">{kind(item)}</span></td>
							<td data-label="About">
								<div class="about">
									{#if item.task_id}<a href={`/work?task=${String(item.task_id)}`}
											>task {item.task_id}</a
										>{/if}
									{#if item.about}<span>{item.about}</span>{/if}
									{#if item.in_reply_to}<Icon name="reply" size={12} /><span class="sr-only"
											>Reply</span
										>{/if}
								</div>
							</td>
							<td data-label="Body"
								><span class="body" title={item.body_preview ?? undefined}
									>{item.body_preview ?? "—"}</span
								></td
							>
						</tr>
					{/each}
		...
		{#if nextCursor && !loading && !failed}
			<div class="more">
				<button type="button" disabled={loadingMore} onclick={() => void read(nextCursor, true)}
					>{loadingMore ? "Loading…" : "Load earlier letters"}</button
				>
			</div>
		{/if}
	</div>
...
<style>
	.comms-log {
		...
		overflow: hidden;
		animation: open 160ms cubic-bezier(0.22, 1, 0.36, 1);
	}
	header {
		min-height: 48px;
		display: flex;
	...
	h2 {
		margin: 0;
		font: 400 1.125rem/1.15 var(--sign);
