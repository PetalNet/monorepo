<script lang="ts">
	...
	const architects = $derived.by(() => {
		const byHost = new SvelteMap<
			string,
			{ host: string; heartbeat: HeartbeatItem; residents: string[] }
		>();
		...
		for (const heartbeat of data.architects) {
			...
			if (heartbeat.handle && !current.residents.includes(heartbeat.handle))
				current.residents.push(heartbeat.handle);
	...
	function peekDisabledReason(heartbeat: HeartbeatItem): string | null {
		if (now - Date.parse(heartbeat.observed_at) > 300_000)
			return "Manager command lane is not answering";
	...
	async function pollPeek(generation: number) {
		...
		try {
			const snapshot = await Effect.runPromise(
				pollTerminalPeek({ stream_id: peekStreamId, tick: peekSeq }),
			);
	...
	async function watchSession(session: HeartbeatItem) {
		...
		try {
			...
			if (generation !== peekGeneration) {
				await Effect.runPromise(closeTerminalPeek({ stream_id: snapshot.stream_id })).catch(
					() => undefined,
				);
				return;
			}
	...
	async function closePeek() {
		...
		peekSession = null;
		if (streamId)
			await Effect.runPromise(closeTerminalPeek({ stream_id: streamId })).catch(() => undefined);
	}
	onMount(() => () => {
		peekGeneration += 1;
		if (pollTimer) clearTimeout(pollTimer);
		if (stallTimer) clearTimeout(stallTimer);
		if (peekStreamId)
			void Effect.runPromise(closeTerminalPeek({ stream_id: peekStreamId })).catch(() => undefined);
	});
</script>
...
<div class="util">
	...
	<div class="flip" role="tablist" aria-label="Agent view">
		<button
			data-view="residents"
			role="tab"
			aria-selected={view === "residents"}
			aria-controls="residents-panel"
			tabindex={view === "residents" ? 0 : -1}
			class:on={view === "residents"}
			onkeydown={flipKey}
			onclick={() => (view = "residents")}>Residents</button
		>
		<button
			data-view="architects"
			role="tab"
			aria-selected={view === "architects"}
			aria-controls="architects-panel"
			tabindex={view === "architects" ? 0 : -1}
			class:on={view === "architects"}
			onkeydown={flipKey}
			onclick={() => (view = "architects")}>Architects</button
		>
	</div>
	<label class="filter">
		...
		<input
			bind:value={filter}
			placeholder={view === "residents" ? "Filter residents" : "Filter architects"}
			aria-label={view === "residents" ? "Filter residents" : "Filter architects"}
		/>
	</label>
	<button
		type="button"
				...
				{#if lane.rows.length > 0}
					...
					{#each lane.rows as row (row.handle)}
						...
						<RosterRow
							...
							peekDisabledReason={residentSession ? peekDisabledReason(residentSession) : null}
			...
			{#each architects as architect (architect.host)}
				<section class="architect">
					<header>
						...
						<div>
							<h2>Architect · {architect.host}</h2>
							<span>Manager supervisor</span>
						</div>
						<StatusPill
							tone={managerStale(architect.heartbeat)
								? "warn"
								: stateTone(architect.heartbeat.state)}
							label={managerStale(architect.heartbeat)
								? `stale ${age(Math.floor(Date.parse(architect.heartbeat.observed_at) / 1_000))}`
								: architect.heartbeat.state.replace("_", " ")}
						/>
					</header>
					<dl>
						<div>
							<dt>PID</dt>
							<dd>{architect.heartbeat.pid}</dd>
						</div>
						<div>
							<dt>Version</dt>
							<dd>{architect.heartbeat.version}</dd>
						</div>
						<div>
							<dt>Uptime</dt>
							<dd>{age(architect.heartbeat.started_at_epoch)}</dd>
						</div>
						<div>
							<dt>Crashes</dt>
							<dd>{architect.heartbeat.crash_count}</dd>
						</div>
						<div>
							<dt>Matrix sync</dt>
							<dd
								class:danger={architect.heartbeat.last_sync_ok_epoch === 0 ||
									now / 1_000 - architect.heartbeat.last_sync_ok_epoch > 120}
							>
								{architect.heartbeat.last_sync_ok_epoch === 0
									? "never"
									: `${age(architect.heartbeat.last_sync_ok_epoch)} ago`}
							</dd>
						</div>
						<div>
							<dt>Observed</dt>
							<dd>{new Date(architect.heartbeat.observed_at).toLocaleTimeString()}</dd>
						</div>
					</dl>
					<div class="residents">
						<span>Residents</span
						>{#each architect.residents as resident, __eachKey18 (__eachKey18)}<a
								href="/agents?agent={resident}">{resident}</a
							>{:else}<em>No resident handle reported</em>{/each}
					</div>
					<footer class="architect-foot">
						<p>
							Architect present. Supervising {architect.residents.length} resident{architect
								.residents.length === 1
								? ""
								: "s"}.
						</p>
						{#if canPeek && architect.heartbeat.tmux_session && architect.heartbeat.pane_id}<button
								type="button"
								disabled={peekDisabledReason(architect.heartbeat) !== null}
								title={peekDisabledReason(architect.heartbeat) ?? "Watch read-only terminal"}
								onclick={() => watchSession(architect.heartbeat)}
								><Icon name="eye" size={13} />Watch session</button
							>{/if}
					</footer>
				</section>
			{:else}
				<div class="unverified">
					<Icon name="circle-help" size={20} />
					<p>
						{data.sources.managers === "unavailable"
							? "Manager heartbeat read unavailable."
							: "No manager heartbeats are visible."}
					</p>
				</div>
			{/each}
		</div>
...
<ModalSurface
	...
	variant="drawer"
	size="wide"
	labelledby="terminal-peek-title"
	onclose={closePeek}
>
	{#if peekSession}
		<div class="peek-drawer">
			...
			<div class="peek-title">
				...
				<div>
					<h2 id="terminal-peek-title">Watch {peekSession.handle ?? "resident"}</h2>
					<p>Read-only live terminal</p>
				</div>
			</div>
			<PTYView
				session={peekSession}
				...
				seq={peekSeq}
				errorCode={peekError}
				onretry={() => watchSession(required(peekSession))}
			/>
		</div>
	{/if}
</ModalSurface>
...
<style>
	...
	.correspondence-toggle:hover {
		background: var(--s2);
		color: var(--text);
	}
	.correspondence-toggle.on {
		background: var(--petal-soft);
		color: var(--petal-text);
	}
	.correspondence-toggle:focus-visible {
		outline: 2px solid var(--petal);
		outline-offset: 2px;
	}
	.strip-wrap {
	...
	.architect-foot button:hover {
		background: var(--s2);
	}
	.architect-foot button:disabled {
		color: var(--text-3);
		cursor: not-allowed;
	}
	.architect-foot button:focus-visible {
		outline: 2px solid var(--petal);
		outline-offset: 2px;
	}
	.residents {
