<script lang="ts">
	...
	import { getLibrarySurface, reviewLibraryCapability, searchLibrary, sendLibraryManagerMessage, updateLibraryStatus, verifyLibraryCapability, type LibraryAcquisitionReceipt, type LibraryManagerAction } from "./library-manager.remote";
	...
	let acquisitions = $state<Partial<Record<string,{state:"loading"|"ready"|"failed";receipt?:LibraryAcquisitionReceipt;message?:string}>>>({});
	const surfaceItems = $derived(lib.items.map((item) => {
	...
	async function reviewProposal(id:string,decision:"under-review"|"promoted"|"rejected"){reviewing=id;try{const result=await Effect.runPromise(reviewLibraryCapability({proposal_id:id,decision,reason:decision==="promoted"?"Reviewed and approved for the shared registry":"Reviewed in the Library curation queue"}));snackbar.push({message:`${result.capability} · ${result.state}`,op:"library.capability.review",tone:decision==="rejected"?"warn":"good"});}catch(error){snackbar.push({message:`Review failed: ${(error as Error).message}`,op:"library.capability.review",tone:"danger"});}finally{reviewing=null;}}
	function open(item: LibraryItemView){ void goto(`/library/${encodeURIComponent(item.id)}`); }
	function selectView(next: LibraryView): void { view = next; }
	async function runSearch(nextQuery:string){query=nextQuery;view="table";if(lib.isMock||!nextQuery.trim()){searchResults=null;searchState="idle";return;}searchState="searching";try{searchResults=await Effect.runPromise(searchLibrary({query:nextQuery}));searchState="idle";}catch{searchResults=[];searchState="failed";}}
...
{#if lib.connected}
{#if view==="list"}<section class="panel review-queue"><header><h2>Capability proposals</h2><span>privileged review gate</span></header><p>{lib.curation?.length??0} reviewable {lib.curation?.length===1?"proposal":"proposals"} in your scopes.</p><small>{lib.sources?.curation==="live"?"Live, scope-filtered Library queue":"Curation source unavailable"}</small><div aria-live="polite">{#each lib.curation??[] as proposal (proposal.id)}<article><div><b>{proposal.capability??proposal.type}{proposal.version?` · v${proposal.version}`:""}</b><small>{proposal.state} · by {proposal.proposedBy??"unknown agent"}</small><p>{proposal.reason}</p></div><div class="review-actions"><button onclick={()=>reviewProposal(proposal.id,"under-review")} disabled={!canReview||reviewing===proposal.id} title={canReview?"Mark as under review":"Admin lane and live Library required"}><Icon name="eye" size={14}/>Review</button><button class="promote" onclick={()=>reviewProposal(proposal.id,"promoted")} disabled={!canReview||reviewing===proposal.id} title={canReview?"Promote to the shared registry":"Admin lane and live Library required"}><Icon name="circle-check" size={14}/>Promote</button><button class="reject" onclick={()=>reviewProposal(proposal.id,"rejected")} disabled={!canReview||reviewing===proposal.id} title={canReview?"Reject proposal":"Admin lane and live Library required"}><Icon name="circle-x" size={14}/>Reject</button></div></article>{:else}<p class="empty">No capability proposals need review.</p>{/each}</div></section>{/if}
{#if view==="list"}
...
{/if}
{/if}
<ModalSurface bind:element={itemDrawer} open={selected!==null} variant="drawer" labelledby="library-item-title" onclose={()=>selected=null}>
	{#if selected}<article class="item-detail"><IconButton class="dialog-close" name="x" label="Close item" autofocus onclick={()=>itemDrawer?.close()}/><header><Icon name="book-open" size={16}/><h2 id="library-item-title">{selected.title}</h2></header><div class="chips"><span>{selected.kind}</span><span>{selected.status}</span><span>{selected.scope}</span><span>conf {selected.confidence?.toFixed(2)??"—"}</span></div><p>{selected.body}</p><section><h3>Typed links</h3>{#each links[selected.id]??[] as link, __eachKey32 (__eachKey32)}<button class="link" onclick={()=>{const target=surfaceItems.find((item)=>item.id===link.targetId);if(target)selected=target}}><Icon name={link.direction==="out"?"arrow-right":"arrow-left"} size={12}/><code>{link.rel}</code><span>{surfaceItems.find((item)=>item.id===link.targetId)?.title??"target unavailable"}</span></button>{:else}<p>No typed links filed.</p>{/each}</section><footer>v{selected.version} · created by {selected.creator}</footer></article>{/if}
</ModalSurface>
<style>
	...
	.item-detail { display:grid; gap:var(--s-3); }
	.item-detail header,.item-detail .chips,.item-detail .link { display:flex; align-items:center; gap:var(--s-2); }
	.item-detail .chips { flex-wrap:wrap; }
	.item-detail .chips span { padding:var(--s-1) var(--s-2); border-radius:var(--r-pill); background:var(--s2); color:var(--text-2); font:500 .6875rem var(--mono); }
	.item-detail section { display:grid; gap:var(--s-2); }
	.item-detail .link { min-height:40px; border:0; border-top:1px solid var(--rule); background:transparent; color:var(--text); text-align:left; }
	.item-detail .link span { min-width:0; overflow-wrap:anywhere; }
	.item-detail footer { color:var(--text-3); font:400 .6875rem var(--mono); }
	...
	.desk .curation{display:none}
	.review-queue{margin:var(--s-3) 0}.review-queue>div{display:grid;gap:var(--s-2)}.review-queue article{display:flex;align-items:flex-start;justify-content:space-between;gap:var(--s-3);padding:var(--s-2) 0;border-top:1px solid var(--rule)}.review-queue article b,.review-queue article small{display:block}.review-queue small{color:var(--text-3);font-size:.6875rem}.review-queue article p{margin:var(--s-1) 0 0;color:var(--text-2);font-size:.75rem}.review-actions{display:flex;flex-wrap:wrap;gap:var(--s-1)}.review-actions button{display:inline-flex;align-items:center;gap:var(--s-1);min-height:32px;padding:0 var(--s-2);border:0;border-radius:var(--r-sm);background:var(--s2);color:var(--text);cursor:pointer}.review-actions .promote{background:var(--petal-soft);color:var(--petal-text)}.review-actions .reject{color:var(--danger-text)}.review-actions button:focus-visible{outline:2px solid var(--petal);outline-offset:2px}.review-actions button:disabled{cursor:not-allowed;opacity:.48}
