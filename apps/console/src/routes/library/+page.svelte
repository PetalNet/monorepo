<script lang="ts">
	...
	async function reviewProposal(id:string,decision:"under-review"|"promoted"|"rejected"){reviewing=id;try{const result=await runRemote(reviewLibraryCapability({proposal_id:id,decision,reason:decision==="promoted"?"Reviewed and approved for the shared registry":"Reviewed in the Library curation queue"}));snackbar.push({message:`${result.capability} · ${result.state}`,op:"library.capability.review",tone:decision==="rejected"?"warn":"good"});}catch(error){snackbar.push({message:`Review failed: ${(error as Error).message}`,op:"library.capability.review",tone:"danger"});}finally{reviewing=null;}}
	...
	async function runSearch(nextQuery:string){query=nextQuery;view="table";if(lib.isMock||!nextQuery.trim()){searchResults=null;searchState="idle";return;}searchState="searching";try{searchResults=await runRemote(searchLibrary({query:nextQuery}));searchState="idle";}catch{searchResults=[];searchState="failed";}}
