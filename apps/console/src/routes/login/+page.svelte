...
<main class="grid min-h-screen place-items-center p-6">
	<section class="card border-base-300 bg-base-200 w-full max-w-md border shadow-xl">
		<div class="card-body gap-6">
			...
			{#if data.authenticated}
				<p class="text-base-content/70">
					Enter the one-time first-run code from the server log to become the owner.
				</p>
				...
				{#if form?.invalid}<p class="alert alert-error">
						That claim code is invalid or has already been used.
					</p>{/if}
			{:else}
				<p class="text-base-content/70">Sign in through the configured SSO provider to continue.</p>
				<button class="btn btn-primary w-full" onclick={signIn}
					><LogIn size={18} /> Sign in with SSO</button
				>
			{/if}
		</div>
	</section>
