...
<ModalSurface bind:element={attachDialog} open={attachOpen} variant="dialog" labelledby="attach-title" onclose={() => attachOpen = false}><div class="terminal-dialog"><h2 id="attach-title">Attach to {active?.host} · {active?.tmux_session}:{active?.pane_id}?</h2><p>You type as {data.adminName}. Every keystroke is logged.</p><div class="dialog-actions"><!-- svelte-ignore a11y_autofocus -- Dialog cancel intentionally receives initial focus. --><button class="ghost" autofocus onclick={() => attachDialog?.close()}>Cancel</button><button class="tonal" disabled={busy} onclick={attachActive}>{busy ? "Attaching…" : "Attach"}</button></div></div></ModalSurface>
