<script lang="ts">
	...
	const age = (epoch: number) => { const seconds = Math.max(0, Math.round(now / 1000 - epoch)); return seconds < 60 ? `${seconds}s` : seconds < 3600 ? `${Math.round(seconds / 60)}m` : seconds < 86400 ? `${Math.round(seconds / 3600)}h` : `${Math.round(seconds / 86400)}d`; };
	const stale = (epoch: number) => now / 1000 - epoch > 30;
	$effect(() => { if (selectedAudit && auditDialog && !auditDialog.open) auditDialog.showModal(); });
	onMount(() => {
