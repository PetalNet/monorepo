...
<header class="sign">
	<h1><Icon name="stamp" size={16} />Correspondence</h1><span>Signals · {pane === "feed" ? "System Bus" : "The Mindy Line"}</span>
		<small class:good={busState === "live"} class:bad={busState !== "live"}><Icon name={busState === "live" ? "circle-check" : "circle-help"} size={13} />{busState === "live" ? `Bus live · heartbeat ${heartbeatAt ? age(heartbeatAt) : "now"} ago${stormFlowCopy ? ` · ${stormFlowCopy}` : ""}` : busState === "gap" ? "Bus gap. Re-read required." : "Can't verify. Bus silent."}</small>
...
<style>
	...
	@media(max-width:767px){.delivery-grid .ladder.wide{display:block}.storm{align-items:flex-start;flex-wrap:wrap}.storm-actions{width:100%;justify-content:flex-end}}
	@media(prefers-reduced-motion:reduce){.storm{transition:none}}
</style>
