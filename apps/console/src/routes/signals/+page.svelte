<script lang="ts">
	...
	function selectPane(next: "feed" | "delivery"): void { pane = next; }
	function updateDeliveryHealth(health: DeliveryLineHealth | null): void { deliveryHealth = health; }
	...
	const age = (ts: string) => { const s = Math.max(0, Math.round((Date.now() - Date.parse(ts)) / 1000)); return s < 60 ? `${String(s)}s` : s < 3600 ? `${String(Math.round(s / 60))}m` : s < 86400 ? `${String(Math.round(s / 3600))}h` : `${String(Math.round(s / 86400))}d`; };
...
<header class="sign">
	...
	<div class="sign-actions"><button class="ghost" onclick={() => { voidOpen = !voidOpen; pane = "feed"; }}><Icon name="search" size={13} />{voidOpen ? "Close the Void" : "Open the Void"}</button><SegmentedControl label="Signals pane" value={pane} options={[{ value: "feed", label: "Feed" }, { value: "delivery", label: "Delivery" }]} onchange={selectPane}/></div>
</header>
