	...
	async function fire(task: TaskItem, op: string, args: Record<string, unknown>, patch?: Partial<TaskItem>, message?: string) {
		...
		try {
			...
			if (data.isMock && patch) {
				...
				tasks = tasks.map((item) => item.id === task.id ? { ...item, ...patch, updated_at: updatedAt } : item);
				if (selected?.id === task.id) selected = { ...selected, ...patch };
				if (patch.status === "done") {
					const completed = { ...task, ...patch, updated_at: updatedAt, settles_at: new Date(Date.parse(updatedAt) + 864e5).toISOString() } as SettlingTask;
...
<section class="board" aria-label="Task board">{#each statuses as column, __eachKey61 (__eachKey61)}<div id={column.id} class="column status-{column.id}" class:review={column.id==="review"} role="list" aria-label={column.label} ondragover={(event)=>{ event.preventDefault(); }} ondrop={() => { if(dragged&&canAct) void move(dragged,column.id); dragged=null; }}><header title={column.id==="blocked"?"Blocked means waiting on a person or a fact. Say which.":undefined}><span>{column.label}</span><b>{visible.filter((task)=>task.status===column.id).length}</b></header>{#each visible.filter((task)=>task.status===column.id).sort((a,b)=>(a.priority-b.priority)||(a.rank??0)-(b.rank??0)).slice(0,expanded[column.id]?undefined:12) as task (task.id)}{@const lease = leaseFor(task.id)}
	...
	<section class="settle" aria-labelledby="settle-title" aria-live="polite">
		...
		{#if settlementQuery.error}
			<div class="settle-error" role="status"><Icon name="circle-alert" size={14}/><span><b>Settlement source unreadable.</b> {settlementQuery.current ? "Last honest rows remain below." : "No completion window can be verified."}</span><button onclick={() => void settlementQuery.refresh()}>Retry</button></div>
		{/if}
		{#if settlementQuery.loading && !settlementQuery.current}
			<div class="settle-skeleton" aria-label="Loading completed work" aria-busy="true"><span></span><i></i></div>
		{:else if settlementQuery.current}
			{#each settlingTasks as task, index (task.id)}<SettleRow {task} {now} newest={index===0} onsettled={settleCompleted}/>{:else}<p>All caught up. Completed work is filed in Library history.</p>{/each}
		{/if}
	</section>
...
<div class="lower"><section class="wanted" aria-labelledby="wanted-title"><header><Icon name="signpost" size={14}/><h2 id="wanted-title">Unclaimed work</h2><span>Soul Squad Board</span><b>{unclaimed.length}</b></header>
	{#if !wantedQuery.current && !data.isMock}
		<div class="wanted-loading" aria-label="Loading wanted board" aria-busy="true">
			{#each Array.from({ length: 4 }, (_, index) => index) as index (index)}<div class="wanted-skeleton"><span></span><i></i></div>{/each}
		</div>
	{:else}
