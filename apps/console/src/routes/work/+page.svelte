<script lang="ts">
	...
	const move = (task: TaskItem, status: Exclude<TaskStatus,"done"|"dropped">) => fire(task, "task.update", { id: task.id, patch: { status }, ...((task.status === "doing" || task.status === "review") && canForce ? { force: true } : {}) }, { status });
	const block = (task: TaskItem) => fire(task, "task.update", { id: task.id, patch: { status: "blocked", blocked_on: blockReason }, ...((task.status === "doing" || task.status === "review") && canForce ? { force: true } : {}) }, { status: "blocked", blocked_on: blockReason });
...
{#snippet taskCard(task: TaskItem)}
	...
	<article class="task" class:urgent={task.priority===0} class:ready={task.status==="review"&&task.verification_status!=="verified"&&!ackedReviewTaskIds.has(task.id)} draggable="true" ondragstart={() => dragged=task}>
		<button class="drill" data-task-card data-status={task.status} onclick={() => { selected=task; rejectReason=""; blockReason=task.blocked_on??""; dispatchRecipient=task.suggested_agent??""; }}><span class="title">{task.title}</span>{#if task.project_title}<span class="project">{task.project_title}</span>{/if}</button>
...
<section class="settle"><header><Icon name="circle-check" size={14}/><h2>Done today</h2><span>Settles to the Library after 24h.</span></header>{#each done as task}<button onclick={()=>{selected=task;blockReason="";dispatchRecipient="";}}><Icon name="circle-check" size={14}/><b>{task.title}</b><span>{task===done[0]?"Done. Points posted.":""}</span><time>settles in {Math.max(0,24-Math.round((now-Date.parse(task.updated_at))/36e5))}h</time></button>{:else}<p>All caught up.</p>{/each}</section>
