<script lang="ts">
	...
	async function confirmRevoke() {
		...
		revokeError = null;
		try {
			await Effect.runPromise(revokeKey({
...
<div class="grid">
	...
	<aside>
		...
		<section><h2>Enrollment <span>Key Ceremony</span></h2>
			...
			{:else}{#each pending as key (key.pubkey_fp)}<CeremonyCard item={key} canAct={canCeremony} disabledReason={ceremonyDisabledReason} onchanged={refreshCeremony} />{:else}<div class="empty"><Icon name="key-round" size={14} /> Nobody at the door.</div>{/each}{/if}
