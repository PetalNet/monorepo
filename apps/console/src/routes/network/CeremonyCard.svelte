<script lang="ts">
	...
	async function approve() {
		...
		try {
			await Effect.runPromise(
				approveEnrollment({ pubkey_fp: item.pubkey_fp, handle: handle.trim() }),
			);
	...
	async function deny() {
		...
		error = null;
		try {
			await Effect.runPromise(denyEnrollment({ pubkey_fp: item.pubkey_fp, reason: reason.trim() }));
...
<article
	class="ceremony-card"
	class:settled={stage === "success"}
	aria-live="polite"
	data-ask={`Pending enrollment ${item.requested_handle ?? "unbound"}, fingerprint ${item.pubkey_fp}, source ${item.source_ip ?? "unknown"}`}
	data-ask-kind="edge-enrollment"
>
	{#if stage === "success"}
		<div class="success">
			<Icon name="circle-check" size={16} /><strong>{success}</strong><span
				>Audited by the named operation.</span
			>
		</div>
	{:else}
		<header>
			...
			<div>
				<strong>{item.requested_handle ?? "Unbound device"}</strong><code title={item.pubkey_fp}
					>{shortFingerprint}</code
				>
			</div>
		</header>
		<dl>
			<div>
				<dt>source</dt>
				<dd><code>{item.source_ip ?? "unknown"}</code></dd>
			</div>
			<div>
				<dt>arrival</dt>
				<dd>{age(item.first_seen_at)}</dd>
			</div>
		</dl>
		...
		{#if stage === "approve"}
			...
			>
				...
				<code class="fingerprint"
					><span>{groups.slice(0, 8).join(" ")}</span><span>{groups.slice(8).join(" ")}</span></code
				>
				<label for={`ceremony-handle-${item.pubkey_fp}`}>Approved handle</label>
				<input
					id={`ceremony-handle-${item.pubkey_fp}`}
					bind:value={handle}
					required
				...
				<div class="actions">
					<button
						class="primary"
						type="submit"
						disabled={!canAct || busy || !handle.trim()}
						title="edge.enroll.approve">{busy ? "Approving…" : "Approve enrollment"}</button
					><button
						class="ghost"
						type="button"
						disabled={busy}
		...
		{:else if stage === "deny"}
			...
			>
				...
				<textarea
					id={`ceremony-reason-${item.pubkey_fp}`}
					bind:value={reason}
					required
					minlength="3"
					maxlength="500"
					rows="3"
					placeholder="Why should this key stay outside?"></textarea>
				<div class="actions">
					<button
						class="danger"
						type="submit"
						disabled={!canAct || busy || reason.trim().length < 3}
						title="edge.enroll.deny">{busy ? "Denying…" : "Deny enrollment"}</button
					><button
						class="ghost"
						type="button"
						disabled={busy}
		...
		{:else}
			<div class="actions">
				<button
					class="tonal"
					disabled={!canAct}
					title={canAct ? "edge.enroll.approve" : disabledReason}
					onclick={() => (stage = "approve")}>Approve enrollment</button
				><button
					class="ghost"
					disabled={!canAct}
					title={canAct ? "edge.enroll.deny" : disabledReason}
					onclick={() => (stage = "deny")}>Deny</button
				>
			</div>
			{#if !canAct}<p class="disabled">
					<Icon name="lock-keyhole" size={12} />{disabledReason}
				</p>{/if}
		{/if}
