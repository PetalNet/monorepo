<script lang="ts">
	...
	import {
		...
		loadInvestigationNode,
		pinInvestigationNode,
	} from "./investigations.remote";
	...
	$effect(() => {
		...
		if (!activeId || !current.some(({ id }) => id === activeId))
			activeId =
				(initialId !== null && current.some(({ id }) => id === initialId)
					? initialId
					: current.at(0)?.id) ?? null;
	});
	...
	async function createRoot(next: InvestigationSeed) {
		...
		try {
			const node = await Effect.runPromise(
				createInvestigationNode({ ...next, parentId: null, parentQuestion: null, scope: null }),
			);
			nodes = [...nodes, node];
			activeId = node.id;
		...
		try {
			const node = await Effect.runPromise(
				createInvestigationNode({
					...
					panelType: ["bar", "line", "stat", "table", "scatter"].includes(panel.type)
						? (panel.type as "bar")
						: "table",
					parentId: active.id,
	...
	async function rerun() {
		...
		action = null;
		snackbar.push({
			message: "Node replayed with current evidence",
			op: "stats.query",
			tone: "good",
		});
	}
	...
	function treeKey(event: KeyboardEvent, id: string) {
		...
		if (event.key === "ArrowDown" || event.key === "ArrowUp") {
			...
			if (next)
				document
					.querySelector<HTMLButtonElement>(`[data-node-id="${CSS.escape(next.id)}"]`)
					?.focus();
		}
		...
		if (event.key === "ArrowLeft") {
			...
			if (row.hasChildren && !collapsed.includes(id)) toggle(id);
				...
				document
					.querySelector<HTMLButtonElement>(`[data-node-id="${CSS.escape(row.parentId)}"]`)
					?.focus();
		}
		if (event.key === "Enter") {
			event.preventDefault();
	...
	function age(iso: string): string {
		...
		return minutes < 60
			...
			: minutes < 1_440
				...
				: `${String(Math.round(minutes / 1_440))}d`;
	}

	function magnitude(panel: InvestigationPanel, row: unknown[]): number {
		const values = panel.rows
			.map((candidate) => candidate.findLast((value) => typeof value === "number"))
	...
	<aside class="tree-rail">
		<header>
			<div>
				...
				<h2 title={nodes.length > 12 ? "Oh, the Time Knife. We've all seen it." : undefined}>
					Reasoning
				</h2>
			</div>
		...
		{:else if rows.length === 0}
			<div class="empty">
				<Icon name="git-branch" size={20} /><b>No investigations yet.</b><span
					>Choose Investigate on an anomaly to preserve its reasoning path.</span
				>
			</div>
		{:else}
				...
				{#each rows as row, index (row.id)}
					<div class="tree-row" style:--depth={row.depth}>
						{#if row.hasChildren}
							<button
								class="disclosure"
								aria-label={collapsed.includes(row.id) ? "Expand branch" : "Collapse branch"}
								onclick={() => {
									toggle(row.id);
								}}
								><Icon
									name={collapsed.includes(row.id) ? "chevron-right" : "chevron-down"}
									size={13}
								/></button
							>
						...
						<button
							...
							onkeydown={(event) => {
								treeKey(event, row.id);
							}}
						>
							<code>{index + 1}</code><span>{row.title}</span><small
								>{row.panelCount}p · {age(row.updatedAt)}</small
							>{#if row.isHome}<Icon name="pin" size={11} title="Pinned as home" />{/if}
						</button>
					</div>
				{/each}
			</div>
	...
	<main class="node-canvas">
		{#if active}
			<nav class="crumbs" aria-label="Investigation ancestry">
				{#each trail as ancestor, index (index)}
					...
					<button class:current={ancestor.id === active.id} onclick={() => (activeId = ancestor.id)}
			...
			<div class="node-toolbar">
				<div>
					<h2>{active.title}</h2>
					<AgentPresence handle={active.createdBy} />
				</div>
				<div class="actions">
					<button disabled={action !== null || loading} onclick={rerun}
						><Icon name="refresh-cw" size={14} />{action === "rerun"
							? "Re-running…"
							: "Re-run"}</button
					>
					<button
						class:pinned={active.isHome}
						disabled={action !== null || active.isHome}
						onclick={pin}
						><Icon name="pin" size={14} />{active.isHome
							? "Pinned home"
							: action === "pin"
								? "Pinning…"
								: "Pin as home"}</button
			...
			{#if error}<div class="error" role="status">
					<Icon name="triangle-alert" size={14} /><span
						>{error}. Last-known tree remains visible.</span
					>
				</div>{/if}
			{#if loading}
				<div class="panel-skeleton" aria-label="Replaying investigation node">
					<i></i><i></i><i></i>
				</div>
			{:else if detail}
				<div class="panels">
					{#each detail.panels as panel (panel.title)}
						<article class="panel">
							...
							{#if panel.refusal}<div class="refusal">
									<Icon name="circle-alert" size={16} /><b>Can’t replay this panel.</b><span
										>{panel.refusal}</span
									><strong>No chart, no guess.</strong>
								</div>
							{:else if panel.rows.length === 0}<div class="empty-panel">
									The replay returned no rows.
								</div>
							{:else if panel.type === "stat"}<div class="stat">
									{formatUnknown(panel.rows[0]?.at(-1) ?? "—")}
								</div>
							{:else}
								<div class="evidence-table" role="table" aria-label={panel.title}>
									<div class="table-head" role="row">
										{#each panel.columns as column, __eachKey48 (__eachKey48)}<span
												role="columnheader">{column}</span
											>{/each}<span></span>
									</div>
									{#each panel.rows as row, __eachKey49 (__eachKey49)}
										<button
											role="row"
											disabled={!panel.queryRef || action !== null}
											onclick={() => drill(panel, row)}
											aria-label={`Drill into ${formatUnknown(row[0] ?? "row")}`}
										>
											{#each row as cell, __eachKey50 (__eachKey50)}<span role="cell"
													>{typeof cell === "number" ? cell.toLocaleString() : String(cell)}</span
												>{/each}
											<i><em style:width={`${String(magnitude(panel, row))}%`}></em></i><span
												class="drill">Drill <Icon name="git-branch" size={12} /></span
											>
										</button>
									{/each}
								</div>
							{/if}
							<footer>
								<Icon name="receipt-text" size={12} /><span
									>{panel.source ?? "source unavailable"} · {panel.rowCount} rows · {panel.observedAt
										? age(panel.observedAt)
										: "freshness unknown"}</span
								><code>{panel.queryRef ?? "no query ref"}</code>
							</footer>
						</article>
					{/each}
				</div>
		...
		{:else}
			<div class="canvas-empty">
				<Icon name="mouse-pointer-2" size={20} /><b>Select a node</b><span
					>Its dashboard will replay here with current, viewer-scoped evidence.</span
				>
			</div>
		{/if}
	</main>
...
<style>
	...
	.tree-row {
		...
		padding-inline-start: calc(var(--depth) * 16px + 4px);
		position: relative;
	}
	.tree-row:not(:last-child)::after {
		content: "";
		position: absolute;
		inset-inline-start: calc(var(--depth) * 16px + 15px);
		top: 28px;
		height: 8px;
	...
	.panel {
		...
		animation: replay var(--dur-mid) var(--ease-standard);
	}
	.panel header {
		display: flex;
		justify-content: space-between;
	...
	.evidence-table > button {
		display: grid;
		grid-template-columns: minmax(120px, 1fr) minmax(80px, 0.6fr) minmax(96px, 1fr) 56px;
		align-items: center;
		gap: var(--s-2);
		min-height: 36px;
		padding: 0 var(--s-2);
	...
	.evidence-table > button:hover:not(:disabled) {
		background: var(--petal-soft);
	}
	.evidence-table i {
		height: 6px;
	...
	.tree-skeleton i:nth-child(2) {
		width: 82%;
		margin-left: 16px;
	}
	.tree-skeleton i:nth-child(3) {
		width: 70%;
		margin-left: 32px;
	}
	.panel-skeleton {
	...
	.panel-skeleton i:first-child {
		width: 38%;
		height: 16px;
	}
	.panel-skeleton i:nth-child(2) {
		height: 128px;
	}
	.panel-skeleton i:last-child {
		height: 12px;
		width: 70%;
	}
	@keyframes replay {
	...
	@media (max-width: 900px) {
		...
		.table-head,
		.evidence-table > button {
			grid-template-columns: minmax(100px, 1fr) 72px 1fr;
		}
		.table-head span:nth-child(n + 3),
		.evidence-table > button > span:nth-child(n + 3):not(.drill) {
			display: none;
		}
	}
