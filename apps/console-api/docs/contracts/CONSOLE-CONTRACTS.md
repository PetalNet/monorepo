...
Feed development mode is a separate, explicit source-level delivery override. The audited
`signal.source_mode{source_service, mode, note?}` operation requires fleet operator access.
`mode:development` suppresses every off-console send from that exact `source.service`—including
P0/safety—without hiding the signal from the in-console feed or Void. `mode:normal` restores the
ordinary subscription, Cocoon, and safety policy. Current state is read from `/signal-sources`;
every transition commits a durable outbox record with the mode change and emits
`signal.source_mode_changed` for live refresh and audit history. Failed appends are retried from
that outbox, so a committed delivery override cannot silently lose its audit event.
...
Staged update approvals are durable audit events. `updates.approve` returns an undo descriptor for
`updates.revoke`; revoke succeeds only while no revoke or apply event exists for that approval.
`GET /api/v1/update-approvals?box_id=…` lists only active, caller-visible approvals so the Updates
surface can render the same reversal after a reload, not only inside the originating snackbar.
