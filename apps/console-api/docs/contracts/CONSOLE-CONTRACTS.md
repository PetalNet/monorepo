...
| `~/.claude/shared/system-outbox/`    | recognized messages → `host.disk.pct`, `container.update_available`, `box.update_status_changed`; unrecognized → `bot.message` | at-least-once; poison/oversize/non-regular records are metadata-only quarantined and emit `bridge.gap_detected`; **the Matrix warning path is NOT decommissioned until `lake.disk.watermark` has fired in anger once** |
